...
const CommonTable = <T extends Record<string, any>>({
  ...
  const columnCount = columns.length + (actions ? 1 : 0) + (selection ? 1 : 0);
      ...
      <div className="rounded-md border bg-card text-card-foreground shadow-sm overflow-hidden">
        <Table>
                ...
                <TableHead className="w-10 px-3">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      selection.onToggleAllVisible(Boolean(checked))
                    }
          ...
          <TableBody>
            {loading ? (
              <TableSkeleton
                columnsCount={columns.length + (selection ? 1 : 0)}
                hasActions={!!actions}
              />
            ) : data.length === 0 ? (
              <TableRow>
              ...
              data.map((row, rowIndex) => (
                (() => {
                  const rowId =
                    selection?.getRowId(row, rowIndex) ?? String(rowIndex);
                  const isSelected =
                    selection?.selectedRowIds.has(rowId) ?? false;
                  ...
                  {selection && (
                    <TableCell className="w-10 px-3">
                      <Checkbox
                        checked={isSelected}
                        onCheckedChange={(checked) =>
                          selection.onToggleRow(rowId, Boolean(checked))
                        }
